export default async function handler(req, res) {
  if (req.method !== "POST") return res.status(405).json({ roadmap: [] });

  try {
    const { skill } = req.body;

    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${process.env.GROQ_API_KEY}`,
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
        model: "llama-3.3-70b-versatile",
        messages: [{
          role: "user",
          content: `Create a structured 7-day learning roadmap for: "${skill}".

Each day should build on the previous one, with practical and specific tasks — not generic advice.

Return ONLY a JSON object in this exact format:
{
  "roadmap": [
    {
      "day": "Day 1",
      "title": "Short, specific topic title",
      "tasks": [
        "Concrete, actionable task 1",
        "Concrete, actionable task 2",
        "Concrete, actionable task 3"
      ]
    }
  ]
}

Rules:
- Exactly 7 days
- Each day has exactly 3 tasks
- Tasks should be specific and actionable, not vague
- Titles should be punchy and specific, not generic
- No markdown, no extra text, just valid JSON`
        }],
        response_format: { type: "json_object" }
      })
    });

    const data = await response.json();

    if (data.error) {
      return res.status(200).json({ roadmap: [], error: `Error: ${data.error.message}` });
    }

    const content = JSON.parse(data.choices[0].message.content);
    res.status(200).json({ roadmap: content.roadmap || content });

  } catch (error) {
    res.status(500).json({ roadmap: [], error: "System error: " + error.message });
  }
}
